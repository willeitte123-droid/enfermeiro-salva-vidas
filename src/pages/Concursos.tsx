...
import { 
  ...
  ExternalLink, Building2, GraduationCap, Filter, AlertCircle 
} from "lucide-react";
...
const Concursos = () => {
  ...
  const filteredConcursos = useMemo(() => {
    return CONCURSOS_MOCK.filter(concurso => {
      ...
      const matchesState = selectedState === "Todos" || concurso.estado.includes(selectedState);
    ...
    <div className="space-y-6 animate-in fade-in duration-700 pb-12">
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="space-y-2 text-center md:text-left">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-xs font-bold uppercase tracking-wider text-emerald-300">
              <Briefcase className="h-3 w-3" /> Oportunidades
            </div>
            ...
            <p className="text-slate-300 max-w-lg text-sm sm:text-base">
              Acompanhe em tempo real os editais abertos e previstos para Enfermagem em todo o Brasil.
            </p>
          </div>
      ...
      <div className="bg-card border rounded-xl p-4 shadow-sm flex flex-col md:flex-row gap-4 items-center">
        <div className="relative w-full md:w-96">
          ...
          <Input 
            placeholder="Buscar órgão ou banca..." 
            value={searchTerm}
        ...
        <div className="ml-auto text-xs text-muted-foreground font-medium whitespace-nowrap">
      ...
      {filteredConcursos.length > 0 ? (
            ...
            <Card key={concurso.id} className="group hover:border-primary/50 transition-all duration-300 hover:shadow-lg flex flex-col">
              <CardHeader className="pb-3">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <Badge variant="secondary" className="font-mono text-xs">{concurso.estado.join(", ")}</Badge>
                </div>
              ...
              <CardContent className="flex-1 space-y-4 text-sm">
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-muted/30 p-2 rounded-lg border border-border/50">
                    ...
                    <p className="font-bold text-emerald-600 dark:text-emerald-400">{concurso.salario}</p>
                  </div>
                  <div className="bg-muted/30 p-2 rounded-lg border border-border/50">
                    <p className="text-[10px] text-muted-foreground uppercase font-bold flex items-center gap-1">
                      <GraduationCap className="w-3 h-3" /> Vagas
                    </p>
                    <p className="font-bold text-foreground">{concurso.vagas}</p>
                  </div>
                </div>
                ...
                <div className="space-y-2 pt-2 border-t border-dashed">
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-muted-foreground flex items-center gap-1.5">
                      <Calendar className="w-3.5 h-3.5" /> Inscrições até:
                    </span>
                    <span className="font-medium text-foreground">
                      {concurso.inscricoesAte ? format(parseISO(concurso.inscricoesAte), "dd/MM/yyyy") : "A definir"}
                    </span>
                  </div>
                  {concurso.dataProva && (
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-medium text-foreground">
                        {format(parseISO(concurso.dataProva), "dd/MM/yyyy")}
                      </span>
                    </div>
                  )}
                </div>
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center bg-muted/20 rounded-2xl border border-dashed">
          ...
          <Button variant="link" onClick={() => {setSearchTerm(""); setSelectedState("Todos");}} className="mt-2">
            Limpar filtros
          </Button>
        </div>
      )}
      ...
      <div className="mt-8 p-4 bg-blue-50 dark:bg-blue-900/10 border border-blue-200 dark:border-blue-800 rounded-lg text-xs text-blue-800 dark:text-blue-300 flex items-start gap-3">
        ...
        <p>
          Esta seção é atualizada periodicamente. Para integração em tempo real com a API de scraping (Python), 
          é necessário configurar o backend e apontar o endpoint no serviço de dados.
        </p>
      </div>
    </div>
  );
